...
load("D:/빅데이터/hadoop/자료/txt/sample data/examples/apple.RData")
load("D:/빅데이터/hadoop/자료/txt/sample data/examples/samsung.RData")

# 총 트윗 수 출력
length(apple.tweets)

#트윗의 자료형 파악
class(apple.tweets)		#list로 출력
...
# 모든 트윗에서 본문글을 출력하는 함수 작성
apple.text<-lapply(apple.tweets,function(t){t$getText()})

# lapply 실행 결과 중에서 3행까지 확인
head(apple.text,3)

pos.word=scan("D:/빅데이터/hadoop/자료/txt/sample data/examples/positive-words.txt",what="character",comment.char=";")
neg.word=scan("D:/빅데이터/hadoop/자료/txt/sample data/examples/negative-words.txt",what="character",comment.char=";")

# 기존의 긍정/부정 사전에 새로운 단어 추가
...
score.sentiment <- function(sentences, pos.words, neg.words, .progress='none'){
  ...
  # 감성기반 분석 후 점수 산출하는 함수
  scores <- laply(sentences, function(sentence, pos.words, neg.words){
    
    # 의미없는 단어(문장부호, 기호등등) 제거
...
# apple에 대한 트윗을 기반으로 감성분석 후 점수 출력
